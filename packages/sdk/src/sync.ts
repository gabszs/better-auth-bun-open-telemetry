...
import { GitHubRepoMetadataSchema, type RepoSource } from "@otel-bun/types";
import { toReferenceName } from "./config.js";
import { getConvexClient, SyncUnavailableError } from "./sync/client.js";

type ConvexApi = typeof import("@otel-bun/sdk/convex/api").api;
...
async function getApi(): Promise<ConvexApi> {
	...
	try {
		const { api } = await import("@otel-bun/sdk/convex/api");
		cachedApi = api;
		return api;
	} catch (error) {
...
export class AuthenticationError extends SyncError {
	constructor(
		message = "Authentication required. Please run 'ow auth login' first.",
	) {
		super(message);
 ...
 */
export async function pullReference(
	fullName: string,
	...
	try {
		...
		client
			.mutation(api.references.recordPull, {
				fullName,
				referenceName: result.referenceName,
			})
			.catch(() => {});
...
export async function pullReferenceByName(
	...
	try {
		...
		client
			.mutation(api.references.recordPull, { fullName, referenceName })
			.catch(() => {});
			...
			switch (result.error) {
				...
				case "rate_limit":
					throw new RateLimitError(
						"Rate limit exceeded. You can push up to 20 times per day.",
					);
				...
				case "invalid_reference":
					throw new InvalidReferenceError(
						result.message ?? "Invalid reference content",
					);
				case "repo_not_found":
...
/**
 * Checks if a repository can be pushed to otel-bun.sh (client-side quick checks)
...
export function canPushToWeb(source: RepoSource): CanPushResult {
	if (source.type === "local") {
		return {
			allowed: false,
			reason:
				"Local repositories cannot be pushed to otel-bun.sh. " +
...
export function validatePushAllowed(source: RepoSource): void {
	...
	if (!result.allowed) {
		const reason: "local" | "not-github" =
			source.type === "local" ? "local" : "not-github";
