...
import type { Config, RemoteRepoSource } from "@otel-bun/types";
...
export function getCommitDistance(
	...
	try {
		...
		const count = execGit(
			["rev-list", "--count", `${olderSha}..${newerSha}`],
			repoPath,
		);
...
async function cloneStandard(
	...
	if (options.shallow) {
		throw new CloneError(
			"Shallow clones are no longer supported. Use a full clone.",
		);
	}
...
async function cloneSparse(
	...
	await execGitAsync(
		["sparse-checkout", "set", ...SPARSE_CHECKOUT_DIRS],
		repoPath,
	);
