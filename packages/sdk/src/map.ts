...
import { GlobalMapSchema, ProjectMapSchema } from "@otel-bun/types/schemas";
...
function readProjectMapSafe(cwd: string): ProjectMap | null {
	const mapPath = resolve(cwd, ".otel-bun/map.json");
	if (!existsSync(mapPath)) return null;
