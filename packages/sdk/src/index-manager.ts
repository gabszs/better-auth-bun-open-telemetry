 ...
 *
 * Manages:
 * - Global map: ~/.local/share/otel-bun/skill/otel-bun/assets/map.json
 * - Project map: ./.otel-bun/map.json
 */
...
/**
 * Reads the global map from ~/.local/share/otel-bun/skill/otel-bun/assets/map.json
 * Returns empty map if file doesn't exist or is invalid
 */
export function readGlobalMap(): GlobalMap {
	const mapPath = Paths.otel-bunGlobalMapPath;
...
/**
 * Writes the global map to ~/.local/share/otel-bun/skill/otel-bun/assets/map.json
 ...
 */
export function upsertGlobalMapEntry(
	qualifiedName: string,
	entry: GlobalMapRepoEntry,
): void {
...
/**
 * Writes a project map to ./.otel-bun/map.json
 *
...
export function writeProjectMap(
	...
	const mapPath = join(projectRoot, ".otel-bun", "map.json");
	const mapDir = dirname(mapPath);
	...
	const projectMap: ProjectMap = {
		version: 1,
		scope: "project",
		globalMapPath: Paths.otel-bunGlobalMapPath,
		repos: entries,
	};
