...
import type { Config } from "@otel-bun/types";
import { ConfigSchema } from "@otel-bun/types";
import { expandTilde, Paths } from "./paths";
...
export function getRepoPath(
	...
	if (!owner || !repo) {
		throw new Error(
			`Invalid fullName format: ${fullName}. Expected "owner/repo"`,
		);
	}
...
export function toReferenceFileName(repoName: string): string {
	if (repoName.includes("/")) {
		...
		const significantPart = repoParts.find(
			(part) => part.length >= 3 && ownerLower.includes(part),
		);
...
export function getReferencePath(fullName: string): string {
	return join(Paths.otel-bunReferencesDir, toReferenceFileName(fullName));
}
...
/**
 * Loads configuration from ~/.config/otel-bun/otel-bun.json
...
/**
 * Saves configuration to ~/.config/otel-bun/otel-bun.json
