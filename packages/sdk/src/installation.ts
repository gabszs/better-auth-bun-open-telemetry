...
import { NpmPackageResponseSchema } from "@otel-bun/types";
import { VERSION } from "./constants.js";

const GITHUB_REPO = "oscabriel/otel-bun";
const NPM_PACKAGE = "otel-bun";
...
/**
 * Detect how otel-bun was installed
 */
export function detectInstallMethod(): InstallMethod {
	...
	const checks: Array<{ name: InstallMethod; test: () => boolean }> = [
		...
		},
		{
			...
			test: () => {
				try {
					execSync("brew list --formula otel-bun 2>/dev/null", {
						encoding: "utf-8",
					});
					return true;
				} catch {
	...
	return new Promise((resolve, reject) => {
		...
		switch (method) {
			case "curl":
				...
				args = [
					"-c",
					`curl -fsSL https://otel-bun.sh/install | VERSION=${version} bash`,
				];
				break;
			case "npm":
				cmd = "npm";
	...
	try {
		...
		for (const line of lines) {
			...
			if (
				...
				if (
					trimmed.includes("# otel-bun") ||
					trimmed === 'export PATH="$HOME/.local/bin:$PATH"'
				) {
					modified = true;
					continue;
				}
