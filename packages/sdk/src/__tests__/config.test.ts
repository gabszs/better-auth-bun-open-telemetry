...
describe("config.ts", () => {
	...
	const home = homedir();
	const configDir = join(home, ".config", "otel-bun").replace(/\\/g, "/");
	const configPath = join(home, ".config", "otel-bun", "otel-bun.json").replace(
		/\\/g,
		"/",
	);
	...
	describe("getMetaRoot", () => {
		it("returns path ending in otel-bun", () => {
			const result = getMetaRoot();
			expect(result).toMatch(/otel-bun$/);
		});

		it("expands ~ to home directory", () => {
			const result = getMetaRoot();
			expect(result).toBe(join(home, ".local", "share", "otel-bun"));
		});
	});
	...
	describe("getMetaPath", () => {
		it("returns {meta}/meta/owner-repo for owner/repo", () => {
			...
			expect(result).toBe(
				join(home, ".local", "share", "otel-bun", "meta", "tanstack-router"),
			);
		});
		...
		it("handles single name (no slash) gracefully", () => {
			...
			expect(result).toBe(
				join(home, ".local", "share", "otel-bun", "meta", "invalid"),
			);
		});
	});

	describe("getConfigPath", () => {
		it("returns ~/.config/otel-bun/otel-bun.json", () => {
			const result = getConfigPath();
			expect(result).toBe(join(home, ".config", "otel-bun", "otel-bun.json"));
		});
	});
	...
	describe("saveConfig", () => {
		...
		it("creates nested directory structure", () => {
			...
			expect(mockMkdirSync).toHaveBeenCalledWith(
				expect.stringMatching(/otel-bun$/),
				{
					recursive: true,
				},
			);
		...
		it("merges with existing config", () => {
			...
			expect(writtenContent.defaultModel).toBe(
	...
	describe("toReferenceFileName", () => {
		it("collapses exact owner/repo match", () => {
			expect(toReferenceFileName("better-auth/better-auth")).toBe(
				"better-auth.md",
			);
		});
		...
		it("collapses when repo part is contained in owner", () => {
			...
			expect(toReferenceFileName("get-convex/convex-backend")).toBe(
				"convex-backend.md",
			);
			expect(toReferenceFileName("get-convex/convex-helpers")).toBe(
				"convex-helpers.md",
			);
