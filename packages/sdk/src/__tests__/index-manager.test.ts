...
import type { GlobalMap, GlobalMapRepoEntry } from "@otel-bun/types";
...
const globalMapPath = vi.hoisted(
	() => "/home/user/.local/share/otel-bun/skill/otel-bun/assets/map.json",
);
const globalMapDir = vi.hoisted(
	() => "/home/user/.local/share/otel-bun/skill/otel-bun/assets",
);
...
vi.mock("../paths.js", () => ({
	Paths: {
		otel-bunGlobalMapPath: globalMapPath,
	},
}));
...
describe("index-manager.ts", () => {
	...
	describe("Paths.otel-bunGlobalMapPath", () => {
		it("returns global map path", () => {
			expect(Paths.otel-bunGlobalMapPath).toBe(globalMapPath);
		});
	});
	...
	describe("readGlobalMap", () => {
		...
		it("returns empty map on schema error", () => {
			addVirtualFile(
				globalMapPath,
				JSON.stringify({
					repos: { "github.com:tanstack/router": { localPath: "/tmp" } },
				}),
			);
	...
	describe("writeGlobalMap", () => {
		it("creates directory when missing", () => {
			...
			expect(mockMkdirSync).toHaveBeenCalledWith(globalMapDir, {
				recursive: true,
			});
		});
	...
	describe("writeProjectMap", () => {
		it("writes project map to .otel-bun/map.json", () => {
			...
			expect(mockMkdirSync).toHaveBeenCalledWith(join(projectRoot, ".otel-bun"), {
				recursive: true,
			});
			const projectMapPath = normalizePath(
				join(projectRoot, ".otel-bun", "map.json"),
			);
