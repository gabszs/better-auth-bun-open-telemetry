...
import type { GlobalMap, ProjectMap } from "@otel-bun/types";
...
const referencesDir = vi.hoisted(
	() => "/home/user/.local/share/otel-bun/skill/otel-bun/references",
);
...
vi.mock("../paths.js", () => ({
	Paths: {
		...
		otel-bunReferencesDir: referencesDir,
	},
}));
...
describe("map.ts", () => {
	...
	describe("getMapEntry", () => {
		...
		it("returns entry from global map", () => {
			...
			expect(result?.entry.localPath).toBe(
		...
		it("prefers project map when it exists", () => {
			addVirtualFile(globalMapPath, JSON.stringify(sampleGlobalMap));
			addVirtualFile(
				normalizePath(process.cwd() + "/.otel-bun/map.json"),
	...
	describe("getProjectMapPath", () => {
		...
		it("returns path when project map exists", () => {
			...
			const mapPath = normalizePath(`${projectPath}/.otel-bun/map.json`);
