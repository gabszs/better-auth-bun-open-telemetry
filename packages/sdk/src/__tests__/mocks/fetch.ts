...
/**
 * Mock response for otel-bun.sh API pull endpoint
 */
export function mockotel-bunPullResponse(reference: {
	fullName: string;
...
/**
 * Mock response for otel-bun.sh API check endpoint
 */
export function mockotel-bunCheckResponse(
	exists: boolean,
	...
	return {
		...
		response: {
			...
			ok: true,
			json: () =>
				Promise.resolve(exists ? { exists: true, ...meta } : { exists: false }),
		},
	};
}
