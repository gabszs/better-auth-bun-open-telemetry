...
vi.mock("../paths.js", () => ({
	Paths: {
		...
		otel-bunReferencesDir: "/mock/data/references",
		otel-bunSkillDir: "/mock/data/skill/otel-bun",
		otel-bunAssetsDir: "/mock/data/skill/otel-bun/assets",
	},
	expandTilde: vi.fn((p: string) => p),
}));

import type { GlobalMap } from "@otel-bun/types";
...
${longContent}
	...
	it("throws when AI response missing reference_output tags", async () => {
		...
		await expect(
			generateReferenceWithAI("/mock/repo", "test/repo"),
		).rejects.toThrow("no valid <reference_output> tags found");
	});
...
${"This is some content ".repeat(50)}
		...
		await expect(
			generateReferenceWithAI("/mock/repo", "test/repo"),
		).rejects.toThrow("must start with markdown heading");
	});
